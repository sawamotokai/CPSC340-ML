...
)  # if using Anaconda, install with `conda install scikit-learn`
...
if __name__ == "__main__":
    ...
    if question == "3.4":
        ...
        print(
            "Our decision tree with DecisionStumpErrorRate took %f seconds"
            % (time.time() - t)
        )
        ...
        print(
            "Our decision tree with DecisionStumpInfoGain took %f seconds"
            % (time.time() - t)
        )
        ...
        for i, max_depth in enumerate(depths):
            ...
            utils.plotClassifier(model, X, y)
            fname = os.path.join("..", "figs", "q6-5.pdf")
        print("scikit-learn's decision tree took %f seconds" % (time.time() - t))

        plt.plot(
            depths, sklearn_tree_errors, label="sklearn", linestyle=":", linewidth=3
        )
